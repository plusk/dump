...
public class CoffeeCupProgram {
	...
	private void part1(){
		...
		cup.drinkCoffee(40); //IllegalArgumentException fra drinkCoffee, capacity = 57.0, currentVolume = 5.0
		cup.drinkCoffee(Math.ceil(r.nextDouble()*42));
		cup.drinkCoffee(Math.floor(r.nextDouble()*20.5));
		cup.fillCoffee(32.5);
		cup.drinkCoffee(Math.ceil(r.nextDouble()*38.9));
	...
	private void part2(){
		...
		cup.fillCoffee(Math.ceil(r.nextDouble()*59)); // IllegalArgumentException fra fillCoffee, [0, 20.5, 14.5, 38.5, 36.5, 6.5, ]
		cup.drinkCoffee(Math.ceil(r.nextDouble()*42));
